...
var generatePolicy = (policy, decoded_token) => {
    console.log(decoded_token);

    policy.allowMethod(AuthPolicy.HttpVerb.POST, "/users/signup");
    policy.allowMethod(AuthPolicy.HttpVerb.POST, "/users/login");
    policy.allowMethod(AuthPolicy.HttpVerb.GET, "/users/verify");

    if (policy.principalId == "unkown") {
        policy.denyMethod(AuthPolicy.HttpVerb.GET, "/products");
    } else {
        policy.allowMethod(AuthPolicy.HttpVerb.GET, "/products");
        policy.allowMethod(AuthPolicy.HttpVerb.POST, "/orders");

        if ("customer_id" in decoded_token) {
            policy.allowMethod(AuthPolicy.HttpVerb.GET, "/users");
        }

        if ("account_id" in decoded_token) {
            policy.allowMethod(AuthPolicy.HttpVerb.GET, "/accounts");
        }
    }
...
exports.handler = (event, context, callback) => {
    ...
    var done = (policy, user_info) => {
        var authResponse = policy.build();
        authResponse.context = {};
        if (user_info && "customer_id" in user_info)
            authResponse.context.customer_id = user_info.customer_id;
        if (user_info && "account_id" in user_info) {
            authResponse.context.account_id = user_info.account_id;
        }
        callback(null, authResponse);
    };

    // parse event.methodArn into (apiOptions, resource, method)
    const apiOptions = {};
    ...
    // validate JWT token
    var token = null;
    if (event.authorizationToken && event.authorizationToken.indexOf('Bearer') > -1) {
    ...
    if (token) {
        jwt.verify(token, process.env.SECRET_KEY, function(err, decoded) {
            ...
            var principalId = (err ? "unkown" : decoded.customer_id);
            var policy = new AuthPolicy(principalId.toString(), awsAccountId, apiOptions);
            if (err) {
                console.log("Authorizer Error:", err);
            }
            generatePolicy(policy, decoded);
            done(policy, err ? {} : decoded);
        });
    } else {
        // create policy for visitors
        var principalId = "unkown";
